...
if __name__ == '__main__':
	...
	for fs in src_pool:
		...
		if not src_fs.sync_with(dst_fs=dst_fs,target_name=transfer_marker_prefix,
			...
			# the youngest three
			src_fs.clean_snapshots(prefix=transfer_marker_prefix,
			...
			for tuple in [["5min",1],
										...
										["weekly",4],
										["monthly",12]]:
				dst_fs.clean_zfs_snapshots(prefix=tuple[0],
			...
			dst_fs.clean_other_snapshots(prefixes_to_ignore=prefixes_to_ignore,number_to_keep=10, dry_run=dry_run, verbose=verbose)
			src_fs.clean_other_snapshots(prefixes_to_ignore=prefixes_to_ignore,number_to_keep=10, dry_run=dry_run, verbose=verbose)
